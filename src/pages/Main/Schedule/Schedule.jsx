...
const Schedule = () => {
	...
	function getData() {
		if (appointment !== null) {
			return appointment?.map(({ id, doctor, patient, date, description, active }, i) => {
				return {
					number: addListNumber(i),
					doctor: showShortName({ last_name: doctor.last_name, name: doctor.name, patronymic: doctor.patronymic }),
					patient: showShortName({ last_name: patient.last_name, name: patient.name, patronymic: patient.patronymic }),
	...
	return (
		<section className={styles.schedule}>
			{modalAppointment || modalAppointmentCreate ?
				...
				>
					{modalAppointment
						...
						: <ModalAppointmentCreate date={date} time={time} handleClick={modalAppointmentCreateOpen} />
					}
				</Stack> : ''}
			...
			>
				...
				<WorkShift modal={setModalAppointmentCreate} getTime={setTime} />
			</Stack>
		</section>
	);
};
